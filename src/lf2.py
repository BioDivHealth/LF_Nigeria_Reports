...
def download_lassa_pdfs():
    ...
    for row in rows:
        ...
        if link and link.lower() != 'wrong' and row.get('Downloaded', '').strip() != 'Y' and (
            row.get('Broken_Link', '').strip() != 'Y' and row.get('Recovered', '').strip() != 'N'
        ):
            try:
...
def organize_pdfs_by_year():
    ...
    for row in rows:
        ...
        if row.get('Downloaded', '').strip() == 'Y' and row.get('Compatible', '').strip() != 'N':
            ...
            file_name = row.get('download_name')
            if file_name:
                src_file = PDF_FOLDER / file_name
                # Destination folder for the specific year
                year_folder = DEST_FOLDER / year
                year_folder.mkdir(parents=True, exist_ok=True)
                dest_file = year_folder / file_name
                if src_file.exists():
                    shutil.copy(src_file, dest_file)
                    print(f"Copied {file_name} to {year_folder}")
                else:
